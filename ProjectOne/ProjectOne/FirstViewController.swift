...
class FirstViewController: UITableViewController, UISearchBarDelegate {
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        if(searchActive){
            ...
            let plainString = myDogList[filteredNames[indexPath.row]]!.pic
            ...
            let plainString = myDogList[dogNamelist[indexPath.row]]!.pic
            let decodedData = NSData(base64EncodedString: plainString, options: NSDataBase64DecodingOptions(rawValue: 0))
            let decodedimage = UIImage(data: decodedData!)
            let image : UIImage = decodedimage! as UIImage
            cell.imageView!.image = image
    ...
    func parsejson(data: NSData){
        do {
            ...
            for item in results {
                ...
                myDogList[name] = Dog(newName: name, newSex: sex, newBreed: breed, newAge: age, newPic: pic)
